...
class report_salary_01_header(models.Model):
    _name = 'aag.report_salary_01_header'
    ...
    detail_ids = fields.One2many(comodel_name="aag.report_salary_01_detail", inverse_name="header_id")

    def action_generate(self):
        cr = self.env.cr

        sql = "delete from aag_report_salary_01_detail where header_id=%s"
        cr.execute(sql, (self.id,))

        sql = """
            INSERT INTO aag_report_salary_01_detail (
                header_id,
                "IDNO",
                "NAME",
                "EMPSTS",
            ...
            SELECT 
                ...
                coalesce((select coalesce(amount,0) from hr_payslip_line where slip_id=ps.id and code='I_PPH_DTP'),0) as I_PPH_DTP,
                ...
                coalesce((select coalesce(amount,0) from hr_payslip_line where slip_id=ps.id and code='D_ACCCOM'),0) as C_ACCCOM,
                coalesce((select coalesce(amount,0) from hr_payslip_line where slip_id=ps.id and code='D_DTHCOM'),0) as C_DTHCOM,
                ...
                coalesce((select coalesce(amount,0) from hr_payslip_line where slip_id=ps.id and code='I_MED_ALW_BAL'),0) as MED_BALANCE,
                coalesce((select coalesce(amount,2) from hr_payslip_line where slip_id=ps.id and code='I_LEAVE_BAL'),2) as LEAVE_BALANCE,
            ...
            from 
                hr_empployee emp 
            join 
                hr_payslip ps on ps.employee_id = emp.id
            join 
    ...
    def action_export(self):
        ...
        self.export_filename = 'report_salary_01_payroll-%s-%s.xlsx' % (self.month, self.year)

        #_logger.info("--- action_export")


class report_salary_01_detail(models.Model):
    _name = 'aag.report_salary_01_detail'
    _description = 'payroll detail'

    header_id = fields.Many2one(comodel_name="aag.report_salary_01_header")
